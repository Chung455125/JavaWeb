...
//	private BigDecimal orderId = new BigDecimal("1");
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		List<Spa> spaList = spaDao.queryAllSpas(); // 所有人預約紀錄
		...
		if(session != null && session.getAttribute("member") instanceof Member) {
			...
			orderList = spaDao.queryOrdersByMemeber(member);
		}
...
//		resp.getWriter().print(isPass + "<p>");
	...
	private void doOrder(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 根據 spaId 取得 Spa 物件
		Spa spa = spaDao.queryAllSpas().stream().filter(sp -> sp.getId().intValue() == spaId.intValue()).findFirst()
				.get();
		// 建立訂單物件
		...
		req.setAttribute("order", order); // 本次預約單
		req.setAttribute("orderList", orderList); // 所有預約單(歷史訂單紀錄)
		rd.forward(req, resp);
	}
}