				...
				<td valign="top">
					<form class="pure-form" method="POST" action="/JavaWeb/rest/user/${ id }">
						<fieldset>
							...
							<input type="text" id="name" name="name" placeholder="請輸入姓名" required="required" value="${ user.name }">	
							<p></p>
							
							<label for="salary">薪資：</label>
							<input type="number" id="salary" name="salary" placeholder="請輸入薪資" required="required" value="${ user.salary }">
							<p></p>
							
							<input type="hidden" name="_method" value="${ _method }" readonly="readonly">
							<p></p>
							
							<button type="reset" 
								onclick="location.href='/JavaWeb/rest/user/'" 
								class="pure-button button-secondary">RESET
							</button>
							<button type="submit" class="pure-button button-succuess">${ _method }</button>
						</fieldset>
					</form>
				</td>
				<%-- User List --%>
